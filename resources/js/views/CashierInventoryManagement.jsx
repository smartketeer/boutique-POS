...
const CashierInventoryManagement = () => {
    ...
    return (
        <div className="space-y-6 max-w-7xl mx-auto">
            ...
            {/* ── Camera Overlay ── */}
            <div role="dialog" aria-modal="true" aria-label="Camera" style={{ position: 'fixed', inset: 0, zIndex: 200, background: '#000', display: 'flex', flexDirection: 'column', opacity: cameraOpen ? 1 : 0, pointerEvents: cameraOpen ? 'auto' : 'none', transition: 'opacity 0.25s ease' }}>
                <canvas ref={canvasRef} style={{ display: 'none' }} />
                {cameraPhase === 'live' ? (
                    <>
                        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(to bottom,rgba(0,0,0,.65) 0%,transparent 100%)' }}>
                        ...
                        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 10, padding: '32px 24px 40px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'linear-gradient(to top,rgba(0,0,0,.7) 0%,transparent 100%)' }}>
                            <div style={{ width: 52 }} />
                ...
                {cameraPhase === 'preview' && capturedPreview ? (
                    <>
                        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 10, padding: 16, display: 'flex', justifyContent: 'center', background: 'linear-gradient(to bottom,rgba(0,0,0,.65) 0%,transparent 100%)' }}><span style={{ color: '#fff', fontWeight: 900, fontSize: 14 }}>Preview</span></div>
                        <img src={capturedPreview.url} alt="Captured" style={{ flex: 1, width: '100%', objectFit: 'cover' }} />
                        <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 10, padding: '24px 24px 40px', display: 'flex', gap: 16, justifyContent: 'center', background: 'linear-gradient(to top,rgba(0,0,0,.75) 0%,transparent 100%)' }}>
