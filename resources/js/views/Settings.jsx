...
import { ShieldCheck, RefreshCw, Save, Loader2, MapPin } from 'lucide-react';
...
const ToggleSwitch = ({ checked, onChange, title, description }) => (
    <div className="flex items-center justify-between py-5 border-b border-zinc-100 last:border-0 gap-4">
        ...
        <button
            ...
            className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus-visible:ring-2 focus-visible:ring-[#818181] focus-visible:ring-offset-2 shadow-inner ${
    ...
    const save = async () => {
        ...
        try {
            const res = await axios.patch('/api/settings', {
                daily_sales_enabled: dailySalesEnabled,
                pos_price_adjustments_enabled: posPriceAdjustmentsEnabled,
                pos_custom_items_enabled: posCustomItemsEnabled,
            });
            setDailySalesEnabled(Boolean(res.data.daily_sales_enabled));
            setPosPriceAdjustmentsEnabled(Boolean(res.data.pos_price_adjustments_enabled));
            setPosCustomItemsEnabled(Boolean(res.data.pos_custom_items_enabled));
            ...
            <div className="bg-white border border-[#cbcbcb] rounded-2xl shadow-sm overflow-hidden">
                <div className="px-5 py-4 sm:px-6 sm:py-5 border-b border-[#cbcbcb] bg-[#dddddd]/50 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2 self-start sm:self-auto">
                        ...
                        <button
                            onClick={save}
                            disabled={saving}
                            className="h-10 px-5 inline-flex items-center gap-2 rounded-xl bg-[#818181] text-white text-sm font-black hover:bg-[#a6a6a6] transition-all disabled:opacity-50 shadow-sm focus:outline-none focus:ring-2 focus:ring-[#818181] focus:ring-offset-2"
                        >
                ...
                <div className="px-6 py-2">
                    ...
                    <ToggleSwitch 
                        ...
                        onChange={setPosPriceAdjustmentsEnabled}
                    />
                    <ToggleSwitch 
                        ...
                        onChange={setPosCustomItemsEnabled}
                    />
                </div>
            </div>
